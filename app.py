...
def validate_json_structure(data):
    ...
    required_fields = ['trackingData', 'deviceDetails']
    if not all(field in data for field in required_fields):
        raise ValueError("Missing required fields in JSON")
    
    if not isinstance(data['trackingData'], list):
        raise ValueError("trackingData must be a list")
    
    return True

def calculate_session_metrics(data):
    """Calculates overall session metrics"""
    attention_data = [item for item in data.get('trackingData', [])
                     if item['type'] == 'FACE_ATTENTION']
    emotion_data = [item for item in data.get('trackingData', [])
                   if item['type'] == 'FACE_EMOTION']
    ...
    metrics = {
        'session_length': max(item['mediaTime'] for item in attention_data) if attention_data else 0,
        'average_attention': np.mean([item.get('attention', 0) for item in attention_data]) if attention_data else 0,
        'emotion_changes': len(set(item.get('dominantEmotion') for item in emotion_data)),
        'total_frames': len(attention_data)
    }
    
    return metrics

def format_download_data(data):
    """Prepares data for download"""
    metrics = calculate_session_metrics(data)
    geo_data = next((item['geoData'] for item in data.get('trackingData', [])
                    if item['type'] == "GEO_INFO"), {})
...
# Add download button in main():
if st.button("📥 Download Processed Data"):
    processed_data = format_download_data(data)
    st.download_button(
        label="Download JSON",
        data=json.dumps(processed_data, indent=2),
        file_name=f"processed_emotion_data_{int(time.time())}.json",
